...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const API = `${API_BASE}/api`; // Verifica que la URL sea correcta
  let productos = [];
  ...
  if (!token) {
    ...
    window.location.href = '/public/inicio/index.html'; // Redirige si no hay token
    return;
  }
  ...
  /*form.imagen.addEventListener('change', e => {
    ...
    return mostrarMensaje('Formato de imagen no válido (solo JPG, PNG, WEBP)', true);
    }
  ...
  form.addEventListener('submit', async e => {
    ...
    if (
      !form.codigo_producto.value.trim() ||
      !form.nombre_producto.value.trim() ||
      !form.precio_min.value.trim()      ||
      !form.cantidad_may.value.trim()          ||
      !form.precio_may.value.trim()            ||
      !form.cantidad_prod.value.trim()
    ) {
    ...
    galeria.innerHTML = !lista.length
      ...
      : lista.map(p => `
        <div class="producto-card">
          ...
          <p>precio_min: $${p.precio_min}</p>
          <p>cantidad_may: $${p.cantidad_may}</p>
          <p>precio_may: $${p.precio_may}</p>
          <p>cantidad_prod: $${p.cantidad_prod}</p
        </div>`).join('');
  }; //<img src="${p.imagen}" alt="${p.nombre_producto}" style="max-width:120px;">
