...
const galeria           = document.getElementById('galeria');
  ...
  const btnFinalizarTransaccion = document.getElementById('finalizarTransaccion');
...
function renderGaleria(lista = productos) {
  ...
  lista.forEach(p => {
    ...
    <img src="${p.imagen || '/imagenes/default.jpg'}" alt="${p.nombre || 'Producto'}" onclick="ampliarImagen('${p.imagen || ''}')">
      ...
      <br>
      <p>Código:                <strong>${p.codigo_producto} </strong></p>
      <p>Precio Minorista:      <strong>$${p.precio_min}     </strong></p>
      <p>Cantidad al Mayorista: <strong>${p.cantidad_may}    </strong></p>
      <p>Precio Mayorista:      <strong>$${p.precio_may}     </strong></p>
      <p>Cantidad de Stock:     <strong>${p.cantidad_prod}   </strong></p>
      <div class="acciones">
        <input
          type="number"
          min="1"
...
function vaciarCarrito() {
  ...
  // 🧹 Limpiar datos en cache (localStorage o sessionStorage)
  localStorage.removeItem('carrito');       // Si guardaste el carrito en localStorage
  sessionStorage.removeItem('carrito');     // Si estaba en sessionStorage

  // 🔄 Volver a renderizar el modal
  renderModalCarrito();

  // ✅ Actualizar el contador del carrito (badge)
  actualizarBadge();

  const totalCarrito = document.querySelector('.carrito-total');
  if (totalCarrito) {
    totalCarrito.textContent = "$0.00"; // o "0 productos"
  }
  // ✅ Mensaje opcional en la consola
  console.log("🗑 Carrito vacío y cache limpiada.");
}
...
// borra individualmente
document.addEventListener('click', (e) => {
  // 🗑 Si hicieron clic en un botón Borrar
  if (e.target && e.target.classList.contains('borrar')) {
    const codigo = e.target.dataset.codigo; // Captura el código del producto
    carrito = carrito.filter(item => item.codigo_producto !== codigo); // Quita ese producto
    renderizarCarrito();        // Redibuja el modal o la lista
...
document.querySelectorAll('.btn-agregar').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    function renderModalCarrito(filtro = '') {
      ...
      productosFiltrados = carrito.filter(item => {
      ...
      totalCantidad = carrito.reduce((acc, item) => acc + item.cantidad, 0);
      totalPrecio = carrito.reduce((acc, item) => acc + (item.precio_unitario * item.cantidad), 0);
...
async function buscarCliente() {
  ...
  try {
    ...
    <div class="cliente-ficha" data-id-cliente="${cli.id_cliente}"></div>
...
//----------------------------------------------------------------------------------------
    ...
    async function generarTicketPDF() {
      ...
      const doc = new jsPDF({
        ...
        unit: "mm",
        format: [100, 200] // Ancho: 80mm (ticket), alto dinámico
      });
    ...
    y += 4;
    doc.line(5, y, 95, y); // línea separadora
    y += 5;
    ...
    carrito.forEach((item) => {
      ...
      const xPUnit = 70;            // Columna Precio Unitario
      const xTotal = 90;            // Columna Total
    ...
    doc.text(`${totalCantidad}`, 90, y, { align: "right" });
    y += 5;
    doc.text(`TOTAL:`, 5, y);
    doc.text(`$${totalPrecio.toFixed(2)}`, 90, y, { align: "right" });
...
//----------------------------------------------------------------------------------------
    ...
    //Transaccion final 
    btnFinalizarTransaccion.addEventListener("click", async () => {
      const confirmar = confirm("¿Estás seguro de finalizar la transacción?");
      if (!confirmar) return;
  
      // 🔥 Obtener forma de pago y tipo de factura directo del DOM
      const formaPago = document.getElementById('seleccionPago')?.innerText.replace('💳 Forma de Pago: ', '').trim();
      const tipoFactura = document.getElementById('seleccionFactura')?.innerText.replace('🧾 Tipo de Factura: ', '').trim();
  
      if (!formaPago || !tipoFactura) {
          alert("⚠️ Debes seleccionar la forma de pago y tipo de factura.");
          return;
      }
  
      if (!cli || !cli.id_cliente) {
          alert("⚠️ Debes buscar un cliente antes de finalizar la transacción.");
          return;
      }
      ...
      if (totalPrecio <= 0 || totalCantidad <= 0) {
          alert("⚠️ El carrito está vacío o los totales no son válidos.");
          return;
      }
      ...
      try {
          ...
          const response = await fetch("http://localhost:4000/api/transacciones", {
            ...
            body: JSON.stringify(dataTransaccion)
        });
          ...
          await guardarDetalleTransaccion(idTransaccion);
  
          alert("✅ Transacción finalizada correctamente.");
